      ...
      <div className="flex flex-col">
        ...
        <div className="flex flex-col font-ibm">
          ...
          <div className="flex mb-4 mt-4">
            <div className="flex align-center text-xl font-bold text-[#000] bg-[#fff] rounded-full w-[26px] h-[26px] justify-center">
              1
            </div>
            <div className="flex flex-col">
              <label className={`ml-2 text-sm font-bold text-[#005]`}>
                Link an authentication app to your XNINJA account
              </label>
              <span className="max-w-[500px]">
                Use a compatible authentication app (like Google Authenticator,
                Authy, Duo Mobile, 1Password, etc.) We’ll generate a QR code for
                you to scan.
              </span>
            </div>
          </div>
          <div className="flex mb-4 mt-4">
            ...
            <div className="flex flex-col">
              ...
              <span className="max-w-[500px]">
                Two-factor authentication will then be turned on for
                authentication app, which you can turn off at any time.
              </span>
            </div>
          </div>
          <div className="flex justify-center">
            <button
              className="mt-4 h-[50px] w-[350px] rounded-full p-2 px-5 text-base font-bold text-white bg-dark hover:bg-dark/90"
              onClick={() => setPage("2fa_page_2")}
            >
              Start
            </button>
          </div>
...
export const SecondPage = ({
  ...
  useLayoutEffect(() => {
    axiosApi
      .post(`${BACKEND_API}/api/auth/two-factor/setup`)
      .then((response) => {
    ...
    >
      <div className="flex flex-col">
        ...
        <div className="flex flex-col font-ibm mt-4">
          ...
          <div className="flex justify-center">
            <button className="mt-4 h-[50px] w-[350px] rounded-full p-2 px-5 text-base font-bold text-white bg-dark hover:bg-dark/90" onClick={() => setPage('2fa_page_3')}>
              Next
            </button>
          </div>
        </div>
...
export const ThirdPage = ({
  ...
  const enable2FA = async () => {
    try {
      await axiosApi.post(`${BACKEND_API}/api/auth/two-factor/enable`, { totpCode });
      setPage('2fa_page_4');
    } catch (error) {
    ...
    >
      <div className="flex flex-col">
        ...
        <div className="flex flex-col font-ibm">
          ...
          <label className="text-xl font-bold mt-4 text-center">You’re all set</label>
...
export const Verify2FA = ({ reloadMe, onVerify }: { reloadMe: () => void; onVerify: (otp: string) => void }) => {
  ...
  const verify2FA = async () => {
    try {
      await axiosApi.post(`${BACKEND_API}/api/auth/two-factor/verify`, { totpCode });
      onVerify(totpCode);
    } catch (error) {
      if (axios.isAxiosError(error) && error.response) {
        if (error.response.status === 403) {
