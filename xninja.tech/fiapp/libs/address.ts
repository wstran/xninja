...
export const getAddressFromPrivateKey = (privateKey: string | undefined = PRIVATE_KEY): string => {
    if (!privateKey) {
...
export const getPublicKeyFromPrivateKey = (privateKey: string | undefined = PRIVATE_KEY): string => {
    if (!privateKey) {
        throw new Error('You have to provide a PRIVATE_KEY in your .env');
    }
...
export const getCosmosPublicKeyFromPrivateKey = (privateKey: string | undefined = PRIVATE_KEY): { type: string; key: string } => {
    if (!privateKey) {
        throw new Error('You have to provide a PRIVATE_KEY in your .env');
    }
