...
export default class GameScene extends Phaser.Scene {
    ...
    create() {
        ...
        for (let x = 0; x < this.gameWidth / CONST.FIELD_SIZE; x++) {
            for (let y = 0; y < this.gameHeight / CONST.FIELD_SIZE; y++) {
                if (y === 0 || y === this.gameHeight / CONST.FIELD_SIZE - 1 || x === 0 || x === this.gameWidth / CONST.FIELD_SIZE - 1) {
                    this.gameBorder[i] = this.add
                        ...
                        .fillRect(CONST.FIELD_SIZE, CONST.FIELD_SIZE, CONST.FIELD_SIZE, CONST.FIELD_SIZE);
                    i++;
                }
            }
        }
    ...
    private rndXPos() {
        return Phaser.Math.RND.between(1, this.horizontalFields - 1) * CONST.FIELD_SIZE;
    }

    private rndYPos() {
        return Phaser.Math.RND.between(1, this.verticalFields - 1) * CONST.FIELD_SIZE;
    }
}
