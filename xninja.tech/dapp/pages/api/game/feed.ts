...
const handler = async ({ req, res, user }: Handler) => {
    ...
    const dbNinja = await ninjaCollection.findOne({ _id, ownerId: user.tw_id }, { projection: { _id: 0, mana: 1, farm_at: 1, class: 1, level: 1 } });
    ...
    const result = await userCollection.findOneAndUpdate({ tw_id: user.tw_id, inventorys: { $exists: true }, [`inventorys.${CONFIG_FOODS[item].food}`]: { $exists: true, $gte: 1 } }, { $inc: { [`inventorys.${CONFIG_FOODS[item].food}`]: -1 } }, { projection: { _id: 0, inventorys: 1 }, returnDocument: 'after' });
