...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
    ...
    if (req.method === "GET") {
        ...
        for (let i = 13; i >= 0; --i) {
            ...
            total_amount.push(guserActionCollection.find({ type: 'upgrade', price_onchain: { $gt: 0 }, created_at: { $lte: new Date(lastDate) } }).project({ price_onchain: 1 }).toArray());

            amount.push(guserActionCollection.find({ type: 'upgrade', price_onchain: { $gt: 0 }, created_at: { $gte: new Date(firstDate), $lte: new Date(lastDate) } }).project({ price_onchain: 1 }).toArray());
