...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
    ...
    if (req.method === "GET") {
        ...
        let total_claim_amount: any[] = await guserClaimCollection.find({ status: 'success', type: 'earn' }).toArray();
        ...
        for (let i = 13; i >= 0; --i) {
            ...
            total_amount.push(guserClaimCollection.find({ status: 'success', type: 'earn', created_at: { $lte: new Date(lastDate) } }).project({ amount: 1 }).toArray());

            amount.push(guserClaimCollection.find({ status: 'success', type: 'earn', created_at: { $gte: new Date(firstDate), $lte: new Date(lastDate) } }).project({ amount: 1 }).toArray());
