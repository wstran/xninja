...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
    ...
    if (req.method === "GET") {
        ...
        let total_spent: any[] = await Promise.all([
            guserMarketCollection.find({ type: { $in: ['buy_food'] }, price_onchain: { $gt: 0 } }).project({ price_onchain: 1 }).toArray(),
        ]);
        ...
        for (let i = 13; i >= 0; --i) {
            ...
            total_amount.push(guserMarketCollection.find({ type: { $in: ['buy_food'] }, price_onchain: { $gt: 0 }, created_at: { $lte: new Date(lastDate) } }).project({ price_onchain: 1 }).toArray());

            amount.push(guserMarketCollection.find({ type: { $in: ['buy_food'] }, price_onchain: { $gt: 0 }, created_at: { $gte: new Date(firstDate), $lte: new Date(lastDate) } }).project({ price_onchain: 1 }).toArray());
        ...
        for (let i = 0; i < total_amount.length; ++i) {
            total_amount[i] = (total_amount[i] as any[]).reduce((prev, current) => prev + current.price_onchain, 0);
        };

        for (let i = 0; i < amount.length; ++i) {
            amount[i] = (amount[i] as any[]).reduce((prev, current) => prev + current.price_onchain, 0);
        };

        total_spent = total_spent.reduce((prev, current) => prev + current.price_onchain, 0);
