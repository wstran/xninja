...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
    ...
    if (req.method === "GET") {
        ...
        let total_spent: any[] = await Promise.all([
            guserMarketCollection.find({ type: { $in: ['buy_chest'] }, price_onchain: { $gt: 0 } }).project({ price_onchain: 1 }).toArray()
        ]);
        ...
        for (let i = 13; i >= 0; --i) {
            ...
            total_amount.push(guserMarketCollection.find({ type: { $in: ['buy_chest'] }, price_onchain: { $gt: 0 }, created_at: { $lte: new Date(lastDate) } }).project({ price_onchain: 1 }).toArray());

            amount.push(guserMarketCollection.find({ type: { $in: ['buy_chest'] }, price_onchain: { $gt: 0 }, created_at: { $gte: new Date(firstDate), $lte: new Date(lastDate) } }).project({ price_onchain: 1 }).toArray());
