                ...
                <button
                    ...
                    onClick={() => {
                        if (!referral_code || !referral_code.startsWith('xninja_')) {
                            toast.error('Code not found!', { duration: 3000, className: 'font-ibm text-xs' });
                            return;
                        }

                        if (referral_code === 'xninja_invite') {
                            toast(
                                `A limited number of 1,000 Ninjas has already joined via this code.
                        Please find other codes in community of Ninjas in our X or discord.`,
                                { duration: 3000, className: 'font-ibm text-xs' }
                            );
                            return;
                        }

                        axiosApi
                            .post(`${import.meta.env.VITE_API_URL}/api/import_referral_code`, { referral_code })
                            .then(reloadMe)
                            .catch((error) => {
                                if (error.response.status === 403) signOut(reloadMe);
                                if (error.response.status === 404) toast.error('Code not found!', { duration: 3000, className: 'font-ibm text-xs' });
                            });
                    }}
                >
                    Proceed
